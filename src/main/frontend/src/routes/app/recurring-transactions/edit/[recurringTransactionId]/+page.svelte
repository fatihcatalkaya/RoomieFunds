<script lang="ts">
	...
	import {
		deleteApiRecurringTransactionByRecurringTransactionId,
		getApiAccount,
		patchApiRecurringTransactionByRecurringTransactionId,
		type GetRecurringTransactionDto,
...
<dialog class="modal" bind:this={modalElement}>
	<div class="modal-box">
		...
		<p class="py-4">
			Bist du dir sicher, dass du Dauerauftrag {recurringTransaction.id} löschen willst?
		</p>
		<div class="modal-action">
			<form method="dialog" class="join">
				<button class="btn btn-error join-item" onclick={reallyDeleteRecurringTransaction}
					>Löschen</button
				>
...
{#if !error}
	<div class="my-4 inline-flex w-full items-center">
		...
		<button class="btn btn-error m-0 h-8 w-8 p-0 text-lg" onclick={deleteRecurringTransaction}>
			<MdiDelete />
		</button>
	</div>
	...
	<form method="dialog" class="grid grid-cols-1 gap-2">
		...
		<div class="join mt-2 w-full">
			<a href="/app/recurring-transactions" class="join-item btn btn-warn w-1/2"> Zurück </a>
			<input
