...
<div class="my-4 inline-flex w-full items-center gap-1">
	<h1 class="flex-grow text-2xl font-bold">Personen</h1>
	<button class="btn btn-warning m-0 h-8 w-8 p-0 text-lg" onclick={sendAccountStatementsNow}>
		<MdiEmailFast />
	</button>
	<a
		href="/app/persons/log"
				...
				{#each persons! as person}
					<tr>
						...
						<td class="text-center">
							<a
								href="/app/persons/edit/{person.id}"
								title="Person {person.id} bearbeiten"
								class="btn btn-primary m-0 h-8 w-8 p-0 text-lg"
							>
								<MdiPencil />
							</a>
							<a
								href="/app/accounts/transactions/{person.accountId}"
								title="Person {person.id} bearbeiten"
