<script lang="ts">
	...
	async function deleteReceipt(transactionId: number) {
		const query = await deleteApiTransactionByTransactionIdReceipt({
	...
	async function fetchAccountStatement() {
		let query = await getApiAccountByAccountIdStatement({
			path: {
				accountId: Number(page.params.accountId!)
			}
		});
...
<dialog class="modal" bind:this={transactionDeleteModal}>
	<div class="modal-box">
		...
		<p class="py-4">
			Bist du dir sicher, dass du Transaktion {transactionDeleteData.transaction?.id} löschen willst?
		</p>
		<div class="modal-action">
			<form method="dialog" class="join">
				<button
					class="btn btn-error join-item"
					onclick={() => reallyDeleteTransaction(transactionDeleteData)}>Löschen</button
				>
...
{:then [transactions, account]}
	<div class="my-4 inline-flex w-full items-center gap-2">
		<h1 class="pr-2 text-2xl font-bold">
		...
		{#if account.active}
			<div class="badge badge-lg badge-primary hidden font-bold md:block">Aktiv</div>
		{:else}
			<div class="badge badge-lg badge-primary hidden font-bold md:block">Passiv</div>
		{/if}
		<div class="flex-grow"></div>
		<button class="btn btn-warning m-0 h-8 w-8 p-0 text-lg" onclick={fetchAccountStatement}>
			<MdiPrinter />
		</button>
		<a
			href="/app/accounts/log/{account.id}"
			title="Änderungsprotokoll"
			class="btn btn-primary m-0 h-8 w-8 p-0 text-lg"
		>
			<MdiScriptText />
		</a>
		<a href="/app/persons/create" class="btn btn-warning m-0 h-8 w-8 p-0 text-lg">
			<MdiPencilPlus />
		</a>
		<button class="btn btn-error m-0 h-8 w-8 p-0 text-lg" disabled>
			<MdiDelete />
		</button>
	</div>
