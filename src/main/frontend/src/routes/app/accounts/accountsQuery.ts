import { getApiAccountWithBalances, type Account } from '$lib/client';
...
function buildTree(accounts: Account[]): TreeNode[] {
	...
	const insertToTree = (path: string[], account: Account, node: TreeNode[]) => {
		...
		if (path.length > 1) {
			insertToTree(path, account, currentNode.children as TreeNode[]);
		} else {
...
export function createAccountsQuery(includeDisabled: boolean): Promise<TreeNode[]> {
	return (async () => {
		const accountQuery = await getApiAccountWithBalances({
			query: {
