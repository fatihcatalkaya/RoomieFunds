...
references:
  - >-
    https://docs.oidc-spa.dev/integration-guides/usage.md?displayAgentInstructions=false
ordinal: 1000
---
...
The Svelte SPA currently pins its OIDC issuer URI and client ID at build time via Vite env vars (VITE_OIDC_ISSUER_URI / VITE_OIDC_CLIENT_ID) and uses oidc-spa v6. That forces a rebuild per environment and blocks us from upgrading to the v10 API.

Move the OIDC public configuration to the backend so a single frontend bundle works in every environment, and upgrade the frontend to oidc-spa v10 using its Framework Agnostic Adapter (the SPA is SvelteKit with adapter-static, so no framework-specific adapter applies).

Backend: a new unauthenticated REST controller exposes the public OIDC parameters the browser needs (issuer URI and client ID). The values are sourced from environment variables, with the docker-compose Keycloak as the local default.

Frontend: fetch that endpoint at bootstrap, feed it into oidc-spa v10 createOidc, and update every call site to the v10 API surface.
